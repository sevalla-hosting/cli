...
export const staticSitesUpdateCommand = makeUpdateCommand({
  ...
  options: [
    ...
    {
      flags: '--allow-deploy-paths <paths>',
      description: 'Glob patterns for paths that trigger deploy (comma-separated)',
    },
    {
  ...
  apiCall: (client, id, opts) => {
    ...
    if (opts['allowDeployPaths'])
      body['allow_deploy_paths'] = (opts['allowDeployPaths'] as string)
        .split(',')
        .map((s: string) => s.trim());
    if (opts['ignoreDeployPaths'])
